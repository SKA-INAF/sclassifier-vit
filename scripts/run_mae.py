...
# - Configure wandb
os.environ["WANDB_PROJECT"]= "vitmae"
...
class DataTrainingArguments:
    ...
    train_dataset: Optional[str] = field(default=None, metadata={"help": "Path to train dataset json file."})
    val_dataset: Optional[str] = field(default=None, metadata={"help": "Path to validation dataset json file."})
    augmenter: Optional[str] = field(default="v1", metadata={"help": "Predefined train augmenter to be used {v1,v2} (default=v1"})
    
    zscale: bool = field(
        default=False,
        metadata={"help": "Whether to apply zscale preprocessing to input data"}
    )
    zscale_contrast: Optional[float] = field(default=0.25, metadata={"help": "zscale contrast parameter"})
    grayscale: bool = field(
        default=False,
        metadata={"help": "Load input images in grayscale (1 chan tensor)"}
    )
    resize: bool = field(
        default=False,
...
def main():
    ...
    transform_v2 = T.Compose(
        [
            T.Resize(data_args.resize_size, interpolation=T.InterpolationMode.BICUBIC),
            RandomResizedCrop(data_args.resize_size, scale=(0.5, 1.0), interpolation=InterpolationMode.BICUBIC),
            FlippingTransform(),
            Rotate90Transform(),
    ...
    ds_train= Dataset.from_generator(lambda: PreTrainDatasetGenerator(dataset_train))
    ds_train.set_format(type="torch", columns=["image"])
    
    ds_val= None
    if dataset_val:
        ds_val= Dataset.from_generator(lambda: PreTrainDatasetGenerator(dataset_val))
        ds_val.set_format(type="torch", columns=["image"])
    ...
    #    examples["pixel_values"] = [transform_train(image) for image in examples[image_column_name]]
    #    return examples
    ...
    if training_args.do_train:
        ...
        if data_args.max_train_samples is not None:
            ...
            ds_train= ds_train.shuffle(seed=training_args.seed).select(range(data_args.max_train_samples))
    ...
    if training_args.do_eval:
        ...
        if data_args.max_eval_samples is not None:
            ...
            #)
            ds_val = (
                ds_val.shuffle(seed=training_args.seed).select(range(data_args.max_eval_samples))
            )
            
        # Set the validation transforms
        #ds["validation"].set_transform(preprocess_images)
        #ds_val.set_transform(preprocess_images)
